...
const LOG_FAILED_CASES_ONLY = true

const equation = new Equation({
  equationAsText: 'x * x * (x - 5)^2 = 0',
  unknownVariable: 'x'
})
...
tests.forEach((oneTest) => {
  let   resultText = 'FAIL'
  const equationAsText = oneTest[0]

  if ((equationAsText.indexOf('>') !== -1) ||
      (equationAsText.indexOf('<') !== -1)) {
    // TODO: Handle unequalities.

  } else {
    if (!LOG_FAILED_CASES_ONLY) {
    ...
    if (LOG_FAILED_CASES_ONLY) {
      if (resultText != 'OK') {
        console.log(oneTest[0], '->', oneTest[1], '...', resultText)
      }
    } else {
