...
function _simplifyNode(node) {
  ...
  if (steps.length > 0) {
    node = _unflattenNode(steps[steps.length - 1].newNode)
  }
    ...
    if (polyObj.doAllExponentsAreInteger) {
      if (polyObj.coeffNodes.length === 3) {
        ...
        if (a && b && c) {
          ...
          equation._logStep('quadratic_delta', true, {
            a, b, c, delta, sqrtDelta, x1, x2, deltaSteps, sqrtDeltaSteps
          })
