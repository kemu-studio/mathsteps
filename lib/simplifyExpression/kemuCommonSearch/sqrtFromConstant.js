...
function sqrtFromConstant(node) {
  ...
  if (Node.Type.isNthRoot(node)) {
    ...
    if (Node.Type.isOperator(baseNode, '/')) {
      ...
      if (sqrtFromConstant(sqrtX).hasChanged() ||
          sqrtFromConstant(sqrtY).hasChanged()) {
        // sqrt(x/y) gives sqrt(x) / sqrt(y)
        const newNode = Node.Creator.operator('/', [sqrtX, sqrtY])
        rv = Node.Status.nodeChanged(ChangeTypes.KEMU_ROOT_FROM_FRACTION, node, newNode)
      }
        ...
        if (factorResult) {
          ...
          if (qNode) {
            ...
            rv = Node.Status.nodeChanged(
              ChangeTypes.KEMU_SQRT_FROM_CONST, node, stepNode2, substeps)

          } else {
            // Perfect match: sqrt(x) -> p
            const newNode = _buildResultNode(pNode, args, argIdx, null, expNode)
            rv = Node.Status.nodeChanged(ChangeTypes.KEMU_ROOT_FROM_CONST, node, newNode)
          }
