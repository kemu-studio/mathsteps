...
function _buildRules(rules, scope, options) {
  // TODO: Review it.
  let ruleSet = null // RULES_CACHE[rules]
  ...
  if (!ruleSet) {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      switch (ruleType) {
        ...
        case 'object':
          newRule = {
            l: removeParens(mathsteps.parseText(rule.l)),
            r: removeParens(mathsteps.parseText(rule.r)),
            id: rule.id,
...
*/
  ...
  let repl = rule.replaceFct
           ? rule.replaceFct(node)
           : rule.r
  ...
  if ((rule instanceof SymbolNode) && (rule.name[0] === 'a')) {
    ...
    if (SUPPORTED_CONSTANTS[rule.name]) {
      ...
      if (node instanceof ConstantNode) {
        if (rule.isNegative) {
          if (mathjs.isNegative(node.value)) {
