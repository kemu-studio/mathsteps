...
function _reduceByArgs(topArgs, bottomArgs) {
  ...
  for (let factorTopIdx = 0; factorTopIdx < topArgs.length; factorTopIdx++) {
    for (let factorBottomIdx = 0; factorBottomIdx < bottomArgs.length; factorBottomIdx++) {
      ...
      } else if (Node.Type.isConstant(factorTop) &&
                 Node.Type.isConstant(factorBottom)) {
        // c1 / c2
        const gcdValue = math.gcd(factorTop.value, factorBottom.value)

        if (math.unequal(gcdValue, 1)) {
          ...
          reducedTopNode = Node.Creator.operator('/', [
            Node.Creator.constant(math.divide(factorTop.value, gcdValue)),
            Node.Creator.constant(math.divide(factorBottom.value, gcdValue)),
          ])
          break searchLoop
        }
