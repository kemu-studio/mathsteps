...
const simplifyCore = require('../_common')
...
function nthRoot(node) {
  ...
  if (Node.Type.isOperator(radicandNode)) {
    if (radicandNode.op === '^') {
      ...
      // nthRoot(x/y, n)
      // Try to apply root on numerator (x) and denominator (y).
      const nodeTop    = Node.Creator.nthRoot(radicandNode.args[0], rootNode)
      const nodeBottom = Node.Creator.nthRoot(radicandNode.args[1], rootNode)

      const statusTop    = nthRoot(nodeTop)
      const statusBottom = nthRoot(nodeBottom)

      if (statusTop.hasChanged() || statusBottom.hasChanged()) {
        const newNode = Node.Creator.operator('/', [statusTop.newNode, statusBottom.newNode])
...
function nthRootExponent(node) {
  ...
  if (rootNode.equals(exponentNode)) {
    ...
    const rootModExponentValue = math.mod(rootValue, exponentValue)
    const exponentModRootValue = math.mod(exponentValue, rootValue)

    if (math.equal(rootModExponentValue, 0)) {
      const newRootValue = math.divide(rootValue, exponentValue)
    ...
    } else if (math.equal(exponentModRootValue, 0)) {
      const newExponentValue = math.divide(exponentValue, rootValue)
...
function groupTermsByRoot(node) {
  ...
  if (math.larger(rootNode.value, MAX_ITERS)) {
    return Node.Status.noChange(node)
  }
  ...
  const newTermGroups = termString.map(key => {
    ...
    // TODO: Review it.
    const term  = simplifyCore.kemuFlatten(mathsteps.parseText(key))
    ...
    while (leftover - times > 0) {
      leftover -= times
      args.push(Node.Creator.operator('*', Array(times).fill(term)))
    }
...
function convertMultiplicationToExponent(node) {
  ...
  if (Node.Type.isParenthesis(radicandNode) || Node.Type.isOperator(radicandNode, '+')) {
    const child = radicandNode.content || radicandNode
    ...
    const children = []
    radicandNode.args.forEach(child => {
      if (Node.Type.isParenthesis(child) || Node.Type.isOperator(child)) {
        const grandChild = child.content || child
...
function nthRootConstant(node) {
  ...
  const radicandValue       = radicandNode.value
  const rootValue           = rootNode.value
  ...
  if (math.equal(math.pow(roundedNthRootValue, rootValue), radicandValue)) {
...
function getRootNode(node) {
  ...
  } else if (Node.Type.isFunction(node, 'sqrt')) {
    return Node.Creator.constant(2)
...
function sortNodes(a, b) {
  if (Node.Type.isConstant(a) && Node.Type.isConstant(b)) {
    if (math.larger(a.value, b.value)) {
      return 1

    } else if (math.smaller(a.value, b.value)) {
      return -1
