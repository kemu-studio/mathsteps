...
function step(options) {
  ...
  while (goOn) {
    ...
    for (let i = 0; i < options.poolOfRules.length; i++) {
      const nodeStatus = applyRules(options.poolOfRules[i], options.node, options.expressionCtx)

      if (nodeStatus) {
        options.node = Status.resetChangeGroups(nodeStatus.newNode)
        options.node = simplifyCommon.kemuFlatten(options.node)
        options.node = simplifyCommon.kemuNormalizeConstantNodes(options.node)

        options.onStepCb(nodeStatus)
