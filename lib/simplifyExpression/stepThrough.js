...
function stepThrough(node, debug = false, expressionCtx = null) {
  ...
  const simplifyCtx = {
    ...
    onStepCb: (nodeStatus) => {
      // console.log("[ STEP ]", nodeStatus.changeType, '|', print.ascii(simplifyCtx.node))

      // if (debug) {
      //  logSteps(nodeStatus)
      // }

      simplifyCtx.node = simplifyCommon.kemuFlatten(simplifyCtx.node)
      simplifyCtx.node = simplifyCommon.kemuNormalizeConstantNodes(simplifyCtx.node)

      const newStep = {
        changeType: nodeStatus.changeType,
        oldNode: oldNode,
        newNode: clone(simplifyCtx.node),
  ...
  while (goOn) {
    ...
    const nodeIdx    = simplifyCtx.lastChangedNodeIdx || 'node'
