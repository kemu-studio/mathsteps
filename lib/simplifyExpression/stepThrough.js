...
function applyRules(fct, node, expressionCtx, parentNode) {
  ...
  } else if (Node.Type.isConstant(node) || Node.Type.isSymbol(node)) {
    // Do nothing.

  } else if (Node.Type.isUnaryMinus(node)) {
    status = applyRules(fct, node.args[0], expressionCtx, parentNode)
    if (status.hasChanged()) {
      rv = Node.Status.childChanged(node, status)
    }
