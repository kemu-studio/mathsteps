...
const kemuCommonSearch    = require('./kemuCommonSearch')
...
function _kemuNormalizeConstantNodes(node) {
  if (Node.Type.isConstant(node)) {
    ...
    if (node.args) {
      node.args.forEach((childNode) => {
        _kemuNormalizeConstantNodes(childNode)
      })
    }
  }
}
...
function stepThrough(node, debug = false, expressionCtx = null) {
  ...
  // Make sure we store all constant nodes as bignumber to avoid fake unequals.
  _kemuNormalizeConstantNodes(node)
  ...
  if (!resultNodeFlat.equals(resultNode)) {
    ...
    steps.push({
      changeType: ChangeTypes.KEMU_REMOVE_UNNEDED_PARENTHESIS,
