  ...
  if (!checks.hasUnsupportedNodes(node)) {
    ...
    if (steps.length > 0) {
      ...
      if (oldNodeAsText !== newNodeAsText) {
        steps.push(
          Node.Status.nodeChanged(ChangeTypes.REARRANGE_COEFF, newNode))
      }
    }
  }
...
module.exports = {
  ...
  newApi: function (node, options) {
    let rv    = null
    let steps = stepThrough(node, options)

    if (steps.length > 0) {
      rv = steps[steps.length - 1].newNode

      if (options.onStepCb) {
        steps.forEach((oneItem) => {
          options.onStepCb(oneItem)
        })
      }
