...
function multiplyFractions(node) {
  ...
  const newDenominator = denominatorArgs.length === 1
    ? denominatorArgs[0]
    : Node.Creator.operator('*', denominatorArgs)

  const newNode = Node.Creator.operator('/', [newNumerator, newDenominator])

  // TODO: Why we need below comparison?
  //console.log("COMPARE A", mathsteps.print(newNode), '|', newNode.toString())
  //console.log("COMPARE B", mathsteps.print(node), '|', node.toString())

  // if (newNode.toString() !== node.toString()) {
  if (mathsteps.print(newNode) !== mathsteps.print(node)) {
    return Node.Status.nodeChanged(ChangeTypes.MULTIPLY_FRACTIONS, node, newNode)
