...
function distribute(node) {
  ...
  } else if (checks.isPolynomialToConstantPower(node)) {
    const [base, exponent] = node.args

    let newNode
    const times = parseFloat(exponent.value)

    if (times > 0) {
      newNode = Node.Creator.operator('*', Array(times).fill(base))

      const status = Node.Status.nodeChanged(
        ChangeTypes.EXPAND_POWER, node, newNode, false)

      newNode = Node.Status.resetChangeGroups(status.newNode)

      return distributeAndSimplifyOperationNode(newNode, [status])
    }
...
function distributeAndSimplifyOperationNode(node, substeps = []) {
