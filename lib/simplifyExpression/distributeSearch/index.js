...
function distribute(node, expressionCtx, parentNode) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    return distributeAndSimplifyMultiplication(node, expressionCtx, parentNode)
...
function distributeUnaryMinus(node, expressionCtx, parentNode) {
  ...
  if ((parentNode != null) && (parentNode.op === '^')) {
    ...
    if (Node.Type.isParenthesis(expNode)) {
      // Tree walk function skips parentheses.
      // Avoid fake false node comparison.
      expNode = expNode.content
    }
  ...
  // KEMU: Is really needed?
  // if (!Node.Type.isParenthesis(unaryContent)) {
  //  return Node.Status.noChange(node);
  // }

  const content = unaryContent.content || unaryContent
