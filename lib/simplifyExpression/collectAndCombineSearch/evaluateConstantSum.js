...
function evaluateConstantSum(node) {
  ...
  if (Node.Type.isOperator(constants)) {
    const evaluateStatus = arithmeticSearch(constants.args)
  ...
  // (which is the case if it's a list surrounded by parenthesis)
  if (Node.Type.isOperator(fractions)) {
    const evaluateStatus = addConstantFractions(fractions)
