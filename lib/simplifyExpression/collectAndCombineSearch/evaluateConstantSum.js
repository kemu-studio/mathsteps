const addConstantAndFraction = require('../fractionsSearch/obsolete/addConstantAndFraction')
const addConstantFractions = require('../fractionsSearch/obsolete/addConstantFractions')
...
function evaluateConstantSum(node) {
  ...
  if (Node.Type.isParenthesis(constants) || Node.Type.isOperator(constants)) {
    const constantList = constants.content || constantList
  ...
  // (which is the case if it's a list surrounded by parenthesis)
  if (Node.Type.isParenthesis(fractions) || Node.Type.isOperator(fractions)) {
    const fractionList = fractions.content || fractions
...
function groupConstantsAndFractions(node) {
  ...
  if (constants.length > 1) {
    constants = Node.Creator.operator('+', constants)
  } else {
  ...
  if (fractions.length > 1) {
    fractions = Node.Creator.operator('+', fractions)
  } else {
