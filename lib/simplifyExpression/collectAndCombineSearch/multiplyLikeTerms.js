...
function multiplyNthRoots(node) {
  ...
  const radicands = node.args.map(term => Node.Type.getRadicandNode(term))
  ...
  const rootNode = Node.Type.getRootNode(firstArg)
...
function collectConstantExponents(node) {
  ...
  const newExponent = Node.Creator.operator('+', exponentNodeList)
