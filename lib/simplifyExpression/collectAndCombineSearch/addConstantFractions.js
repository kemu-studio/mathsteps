...
function addConstantFractions(node) {
  ...
  const denominators = node.args.map(fraction => {
    return fraction.args[1].evaluate()
  })
  ...
  if (!denominators.every(denominator => math.equal(denominator, denominators[0]))) {
...
function combineNumeratorsAboveCommonDenominator(node) {
  ...
  const newNumerator = Node.Creator.operator('+', numeratorArgs)
...
function addNumeratorsTogether(node) {
  ...
  newNode.args[0] = Node.Creator.constant(newNode.args[0].evaluate())
  return Node.Status.nodeChanged(
...
function reduceNumerator(node) {
  ...
  if (Node.Type.kemuIsConstantInteger(newNode.args[0], 0)) {
...
function makeCommonDenominator(node) {
  ...
  newNode.args.forEach((child, i) => {
    ...
    const missingFactor = math.divide(commonDenominator, denominators[i])

    if (math.unequal(missingFactor, 1)) {
      const missingFactorNode = Node.Creator.constant(missingFactor)
      const newNumerator  = Node.Creator.operator('*', [child.args[0], missingFactorNode])
      const newDeominator = Node.Creator.operator('*', [child.args[1], missingFactorNode])
...
function evaluateDenominators(node) {
  ...
  newNode.args.map(fraction => {
    fraction.args[1] = Node.Creator.constant(fraction.args[1].evaluate())
  })
...
function evaluateNumerators(node) {
  ...
  newNode.args.map(fraction => {
    fraction.args[0] = Node.Creator.constant(fraction.args[0].evaluate())
  })
