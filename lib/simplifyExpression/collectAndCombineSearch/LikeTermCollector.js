...
LikeTermCollector.collectLikeTerms = function(node) {
  ...
  termTypesSorted.forEach(termType => {
    ...
    if (termsOfType.length === 1) {
      ...
      const termList = clone(Node.Creator.operator(op, termsOfType))
...
function getTermsForCollectingAddition(node) {
  ...
  if ((terms[CONSTANT] != null) &&
      (terms[CONSTANT].length === 1) &&
      (terms[CONSTANT_FRACTION] != null) &&
      (terms[CONSTANT_FRACTION].length === 1)) {
...
function getTermsForCollectingMultiplication(node) {
  ...
  for (let i = 0; i < node.args.length; i++) {
    ...
    if (Node.Type.isUnaryMinus(child)) {
      terms = Util.appendToArrayInObject(terms, CONSTANT, Node.Creator.constant(-1))
      child = child.args[0]
    }
...
function addToTermsforNthRootMultiplication(terms, node) {
  const rootNode      = Node.Type.getRootNode(node)
  const rootNodeValue = rootNode.value

  return Util.appendToArrayInObject(terms, NTH_ROOT + rootNodeValue , node)
}
