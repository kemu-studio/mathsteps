...
function divideByGCD(fraction) {
  ...
  const numeratorValue   = evaluate(fraction.args[0])
  const denominatorValue = evaluate(fraction.args[1])
  ...
  if (math.isNegative(denominatorValue)) {
    gcd = math.multiply(gcd, -1)
  }
...
function findGCD(node, gcd, numeratorValue, denominatorValue) {
  ...
  const intermediateNumerator   = Node.Creator.operator('*', [Node.Creator.constant(math.divide(numeratorValue, gcd)), gcdNode])
  const intermediateDenominator = Node.Creator.operator('*', [Node.Creator.constant(math.divide(denominatorValue, gcd)), gcdNode])
...
function cancelGCD(node, gcd, numeratorValue, denominatorValue) {
  ...
  const newNumeratorNode   = Node.Creator.constant(math.divide(numeratorValue   , gcd))
  const newDenominatorNode = Node.Creator.constant(math.divide(denominatorValue , gcd))

  if (math.equal(newDenominatorNode.value, 1)) {
