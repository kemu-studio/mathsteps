    ...
    for (let factorBottomIdx in bottomArgs) {
      ...
      if (factorTop.equals(factorBottom)) {
        ...
        if (math.unequal(valueGcd, 1)) {
          ...
          if (math.unequal(valueTop, valueGcd)) {
            reducedTopNode = Node.Creator.constant(math.divide(valueTop, valueGcd))
          }

          if (math.unequal(valueBottom, valueGcd)) {
            reducedBottomNode = Node.Creator.constant(math.divide(valueBottom, valueGcd))
          }
        }
