  ...
  if (Node.Type.isNthRoot(node)) {
    ...
    if (Node.Type.isOperator(baseNode, '/')) {
      ...
      if (sqrtFromConstant(sqrtX) || sqrtFromConstant(sqrtY)) {
        ...
        rv = Node.Status.nodeChanged(ChangeTypes.KEMU_ROOT_FROM_FRACTION, newNode)
      }
      ...
      for (let argIdx in args) {
        ...
        if (factorResult) {
          ...
          if (qNode) {
            ...
            // Prepare substeps.
            const substeps = [
              Node.Status.nodeChanged(ChangeTypes.KEMU_FACTOR_EXPRESSION_UNDER_ROOT , stepNode1),
              Node.Status.nodeChanged(ChangeTypes.KEMU_SQRT_FROM_POW                , stepNode2)
            ]

            // Final result.
            rv = Node.Status.nodeChanged(ChangeTypes.KEMU_SQRT_FROM_CONST, stepNode2, substeps)

          } else {
            // Perfect match: sqrt(x) -> p
            const newNode = _buildResultNode(pNode, args, argIdx, null, expNode)
            rv = Node.Status.nodeChanged(ChangeTypes.KEMU_ROOT_FROM_CONST, newNode)
          }
