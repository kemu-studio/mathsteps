...
function calculateNumericalValue(node, expressionCtx) {
  let rv = null
  ...
  if (!expressionCtx || !expressionCtx.isNumerical()) {
    ...
    if (evaluateHandler) {
      ...
      rv = Node.Status.nodeChanged(evaluateHandler.stepId, newNode)
    }
  } else if ((node.op == '/') &&
    ...
    rv = Node.Status.nodeChanged(ChangeTypes.KEMU_NUMERICAL_DIV, newNode)
  }
