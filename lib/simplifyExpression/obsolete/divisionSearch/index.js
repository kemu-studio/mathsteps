...
function simplifyDivisionChain(node) {
  ...
  if (denominatorList.length > 2) {
    ...
    // (b/c) * d      b/c    d     b * d
    //
    // Possible improvement: More general handler?

    const a = node.args[0]
    const b = node.args[1].args[0].args[0]
    const c = node.args[1].args[0].args[1]
    const d = node.args[1].args[1]

    const numerator   = Node.Creator.operator('*', [a, c])
    const denominator = Node.Creator.operator('*', [b, d])
    const newNode     = Node.Creator.operator('/', [numerator, denominator])
