...
EquationOperations.isolateSymbolOnLeftSide = function(equation, symbolName) {
  ...
  // If there is no constant term on the left side of the equation
  // and the left side is not a polynomialTerm
  // or the left side is a symbol, return no change
  // e.g. x = 0, no change
  // e.g. 2x + 3x = 1, no change
  // e.g. x^2 = 1, change
  if (!nonSymbolTerm && !Node.Type.isOperator(leftNode, '^')) {
    return EquationStatus.noChange(equation)
  }

  // Clone it so that any operations on it don't affect the node already
  // in the equation
  nonSymbolTerm = nonSymbolTerm
    ? clone(nonSymbolTerm)
    : null
  ...
  if (Node.Type.isOperator(leftNode)) {
    if (leftNode.op === '+') {
      ...
      inverseOp = 'nthRoot'
      changeType = ChangeTypes.NTH_ROOT_BOTH_SIDES
      inverseTerm = leftNode
    } else {
...
function performTermOperationOnEquation(equation, op, term, changeType) {
  ...
  // The root is the exponent of the polynomial term on the left side
  const root = op === 'nthRoot'
    ? leftTerm.args[1]
    : null

  // If the operation is nthRoot, then the right term is the entire right side of the equation
  const rightTerm = op === 'nthRoot'
    ? equation.rightNode
    : clone(term)

  const leftNode = performTermOperationOnExpression(
    equation.leftNode, op, leftTerm, root)
  const rightNode = performTermOperationOnExpression(
    equation.rightNode, op, rightTerm, root)
...
function performTermOperationOnExpression(expression, op, term, root = null) {
  const node = (Node.Type.isOperator(expression) ?
    Node.Creator.parenthesis(expression) : expression)

  term.changeGroup = 1
  const newNode = op === 'nthRoot'
    ? Node.Creator.nthRoot(term, root)
    : Node.Creator.operator(op, [node, term])
