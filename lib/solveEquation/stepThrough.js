const cancelLikeTerms = require('../simplifyExpression/fractionsSearch')
...
/*
  Given any node, return a dictionary and a list of constant terms.
  The dictionary will map a base to its largest exponent value (if
  the term is a power node, otherwise, the value will be 1)

  e.g. 1/2 + 1/3 -> map : {}, constants: [2, 3]
  e.g. 1/x + 1/x^2 -> map : {x : 2} <- x is the base and 2 is the highest exponent
  with x as the base
  e.g. 1/x + 1/(x+1)^2 -> map: {x: 1, x + 1: 2}
  e.g. 1/2 + 1/(3x^2) -> map: {x: 2}, constants: [2, 3]
*/
function denomMap(terms, map = {}, constants = []) {
  terms.forEach(function(term) {
    if (Node.Type.isConstant(term)
      ...
      denomMap(term.args, map, constants)
    } else {
...
// Given a node, collect all denominators in a list and return it
// e.g. 1/x + 1/2 -> [x , 2]
function getAllDenoms(node, terms = []) {
  node.args.forEach(function(term) {
    term = getContent(term)

    if (Node.Type.isOperator(term, '/')) {
      const denom = getContent(term.args[1])
      terms.push(denom)
    }
  })
  return terms
...
// Helper Functions to get the primeFactorization and lcm of a list of numbers
...
// TODO: negative values in denom
function multiplyBothSidesByCommonDenom(equation) {
  // Collect all the denominators in a given equation
  let terms = getAllDenoms(equation.leftNode)
  terms = getAllDenoms(equation.rightNode, terms)

  const [map, constants] = denomMap(terms)

  // Return a list of all the terms in the newDenom
  // e.g. map: {'x + 1': 2, 'x': 3} -> [(x+1)^2, x^3]
  const newDenomTerms = Object.keys(map).map(key => {
    const base = math.parse(key)
    const exponent = math.parse(map[key])
    return map[key] === 1
      ? base
      : Node.Creator.operator('^', [base, exponent])
  })

  // Lowest common multiple of all the constant terms
  const LCM = Node.Creator.constant(lcm(...constants))

  // Generate common denominator from the LCM and newDenomTerms
  let commonDenom
  if (LCM.value > 1) {
    if (newDenomTerms.length > 1) {
      commonDenom = Node.Creator.operator('*', [LCM, ...newDenomTerms])
    } else {
      commonDenom = Node.Creator.operator('*', [LCM, newDenomTerms[0]])
    }
  } else {
    if (newDenomTerms.length > 1) {
      commonDenom = Node.Creator.operator('*', [...newDenomTerms])
    } else {
      commonDenom = newDenomTerms[0]
    }
  }

  const newEquation = clone(equation)
  newEquation.leftNode = Node.Creator.operator('*', [commonDenom, newEquation.leftNode])
  newEquation.rightNode = Node.Creator.operator('*', [newEquation.rightNode, commonDenom])

  const test = Node.Creator.operator('/', [commonDenom, terms[0]])
  const simplified = cancelLikeTerms(test)
  console.log(print(simplified.newNode))
  return new EquationStatus(ChangeTypes.MULTIPLY_BOTH_SIDES_BY_COMMON_DENOMINATOR, equation, newEquation)
}
...
function stepThrough(leftNode, rightNode, comparator, debug = false) {
  ...
  /*
    Check if there is a fraction in the leftNode of an equationStatus
    // TODO: maybe right?
    */
  const hasFraction = equation.leftNode.args.some(function(arg) {
    return Node.Type.isOperator(getContent(arg), '/')
  })

  if (hasFraction) {
    equationStatus = multiplyBothSidesByCommonDenom(equation)

    if (equationStatus.hasChanged()) {
      steps.push(equationStatus)
      equation = equationStatus.newEquation
    }
  }
...
function getSolutionsAndSymbol (equation) {
  ...
  */
  ...
  for (let f in factorsWithSymbols) {
