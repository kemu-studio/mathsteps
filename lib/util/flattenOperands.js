...
function flattenOperands(node) {
  ...
  // TODO: Why we need clone here?
  // TODO: flattenOperands(-q - 1/2) gives -q + 1/2 without clone.
  node = clone(node)
  ...
  if (Node.Type.isConstant(node, true)) {
    ...
    const newNode = Node.Creator.operator('*', [flattenedFn, flattenedArg])
    return newNode
    ...
    // Possible improvement: More general way to flatten function args.
    node.args[0] = flattenOperands(node.args[0])
    return node
