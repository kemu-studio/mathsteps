    ...
    node.args.forEach((nextItem) => {
      ...
      switch (op) {
        ...
        case '*': {
          ...
          wrapItemIntoParenthesis = (nextItem.op === '+') || (nextItem.op === '-')
        ...
        case '/': {
          if (Node.Type.isOperator(nextItem)) {
            // (...) / (...)
            wrapItemIntoParenthesis = true

          } else if (prevItem && Node.Type.isConstant(prevItem) && Node.Type.isConstant(nextItem)) {
            // 1/2
            opForPresentation = '/'
          }
