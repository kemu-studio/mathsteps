...
function _isThereDigitAt(str, idx) {
...
var _createUtil = (0, _util.createUtil)({
  ...
  OperatorNode: mathjs.OperatorNode,
  SymbolNode: mathjs.SymbolNode
})
    ...
    switch (node.op) {
      ...
      case '*': {
        node.args.forEach((nextItem) => {
          ...
          if (!(Node.Type.isConstant(prevItem) && !Node.Type.isConstant(nextItem))) {
            rv += op
          }
      ...
      case '^': {
        ...
        let xForPresentation = printAscii(x)
        let yForPresentation = printAscii(y)

        if (Node.Type.isOperator(x)) {
          xForPresentation = '(' + xForPresentation + ')'
        }

        rv = xForPresentation + '^' + yForPresentation
...
function xprintAscii(node, showPlusMinus = false) {
