...
class Term {
  ...
  constructor(node, baseNodeFunc, onlyImplicitMultiplication=false) {
    const values = this.parseNode(node, baseNodeFunc, onlyImplicitMultiplication);
    this.base = values.base;
    this.exponent = values.exponent;
    this.coeff = values.coeff;
  }

  // Parses a given term node and returns the term's
  // base, exponent, and coefficient.
  // e.g. 3x would return {symbol: 'x', exponent: 1, coeff: 3}
  // If the node is not a term, it will throw an error
  parseNode(node, baseNodeFunc, onlyImplicitMultiplication) {
