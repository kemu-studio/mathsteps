...
class Term {
  ...
  constructor(node, baseNodeFunc, onlyImplicitMultiplication = false) {
    ...
    // TODO: x - 10/13x becames x - (-10/13x) without clone.
    node = clone(node)
...
function negativeCoefficient(node) {
  if (NodeType.isConstant(node)) {
    // Node is a constant
    node = NodeCreator.constant(math.subtract(0, node.value))

  } else {
    // Node is a constant fraction
    const numeratorValue = math.subtract(0, node.args[0].value)
