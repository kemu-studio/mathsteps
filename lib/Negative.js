...
Negative.isNegative = function(node) {
  if (NodeType.isUnaryMinus(node)) {
    ...
    return math.isNegative(node.value)

  } else if (NodeType.isConstantFraction(node)) {
    const numeratorValue        = node.args[0].value
    const denominatorValue      = node.args[1].value
    const isNumeratorNegative   = math.isNegative(numeratorValue)
    const isDenominatorNegative = math.isNegative(denominatorValue)
...
Negative.negate = function(node, naive = false) {
  if (NodeType.isConstantFraction(node)) {
    ...
    if (NodeType.isUnaryMinus(node)) {
      ...
      return NodeCreator.constant(math.subtract(0, node.value))
    }
  }

  // x/y -> -x/y
  if ((node.op === '/') && node.args[0]) {
    // Negate numerator.
    node.args[0] = Negative.negate(node.args[0], naive)
    return node
  }
...
Negative.negatePolynomialTerm = function(node, naive = false) {
  ...
  if (!polyNode.hasCoeff()) {
    ...
    if (NodeType.kemuIsConstantInteger(oldCoeff, -1)) {
      newCoeff = null
      ...
      if (NodeType.kemuIsConstantInteger(newCoeff, 1)) {
        newCoeff = null
      }
    }
  }
