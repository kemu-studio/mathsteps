...
function factorQuadratic(node) {
  ...
  for (const term of node.args) {
    if (Node.Type.isConstant(term)) {
      ...
      if (Node.Type.kemuIsConstantInteger(exponent, 2)) {
        symbol = polyTerm.getSymbolNode()
        aValue = polyTerm.getCoeffValue()
      } else if (Node.Type.kemuIsConstantInteger(exponent, 1)) {
  ...
  if (!symbol || math.equal(aValue, 0)) {
    return Node.Status.noChange(node)
  }

  let negate = false

  if (math.isNegative(aValue)) {
    negate = true
    aValue = math.multiply(-1, aValue)
    bValue = math.multiply(-1, bValue)
    cValue = math.multiply(-1, cValue)
  }
...
function factorSymbol(node, symbol, aValue, bValue, cValue, negate) {
  if (math.equal(bValue, 0) || math.unequal(cValue, 0)) {
    return Node.Status.noChange(node)
  }

  const gcd     = math.gcd(aValue, bValue)
  const gcdNode = Node.Creator.constant(gcd)
  const aNode   = Node.Creator.constant(math.divide(aValue, gcd))
  const bNode   = Node.Creator.constant(math.divide(bValue, gcd))
  ...
  const paren        = Node.Creator.operator('+', [polyTerm, bNode])
...
function factorDifferenceOfSquares(node, symbol, aValue, bValue, cValue, negate) {
  ...
  aValue = math.divide(aValue, gcd)
  cValue = math.divide(cValue, gcd)

  const aRootValue = math.sqrt(math.abs(aValue))
  const cRootValue = math.sqrt(math.abs(cValue))

  // must be a difference of squares
  if (math.isInteger(aRootValue) &&
      math.isInteger(cRootValue) &&
      math.isNegative(cValue)) {
    ...
    const firstParen  = Node.Creator.operator('+', [polyTerm, cRootNode])
    const secondParen = Node.Creator.operator('-', [polyTerm, cRootNode])

    // create node in difference of squares form
    let newNode = Node.Creator.operator('*', [firstParen, secondParen], true)

    if (math.unequal(gcd, 1)) {
      const gcdNode = Node.Creator.constant(gcd)
...
function factorPerfectSquare(node, symbol, aValue, bValue, cValue, negate) {
  ...
  if (math.equal(bValue, 0) || math.equal(cValue, 0)) {
    return Node.Status.noChange(node)
  }
  ...
  if (math.isNegative(bValue)) {
    cRootValue = math.multiply(cRootValue, -1)
  }

  // apply the perfect square test
  const perfectProduct = math.multiply(2, aRootValue, cRootValue)

  if (math.isInteger(aRootValue) &&
      math.isInteger(cRootValue) &&
      math.equal(math.divide(bValue, gcd), perfectProduct)) {

    const aRootNode = Node.Creator.constant(aRootValue)
    const cRootNode = Node.Creator.constant(cRootValue)

    const polyTerm = Node.Creator.polynomialTerm(symbol, null, aRootNode)
    const paren    = Node.Creator.operator('+', [polyTerm, cRootNode])
...
function factorSumProductRule(node, symbol, aValue, bValue, cValue, negate) {
  ...
  if (math.unequal(bValue, 0) && math.unequal(cValue, 0)) {
    // we factor out the gcd first, providing us with a modified expression to
    ...
    for (const pair of factorPairs) {
      if (math.equal(math.add(pair[0], pair[1]), bValue)) {
        ...
        const firstTerm  = Node.Creator.operator('+', [ax2, px])
        const secondTerm = Node.Creator.operator('+', [qx, c])
        ...
        const r  = Node.Creator.constant(math.divide(aValue, u.value))
        const s  = Node.Creator.constant(math.divide(pValue, u.value))
        ...
        const firstParen = Node.Creator.operator('+', [rx, s])
        ...
        if (math.isNegative(qValue)) {
          vValue = math.multiply(vValue, -1)
        }

        const v = Node.Creator.constant(vValue)

        // create the second parenthesis
        const secondParen = Node.Creator.operator('+', [ux, v])
