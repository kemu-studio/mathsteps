...
EquationSolveManager.registerSolveFunction('C=C', (equation) => {
  ...
  if (Node.Type.isConstant(diffNode)) {
    ...
    if (Node.Type.isZero(diffNode)) {
      // L - R = 0
      // Solution is true.
      equation.solve(NODE_TRUE)
    } else {
      // L - R != 0
      // Solution is false.
      equation.solve(NODE_FALSE)
    }
...
const equation = Equation.createFromString('2a+a=3a', 'x')
